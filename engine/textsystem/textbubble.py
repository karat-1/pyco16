...
class Textbubble:
    ...
    def render(self, surf: pygame.Surface, offset, font: dict):
        x_offset = 5 * 3
        y_offset = 5 * 3
        textbox_surf = pygame.Surface((self.game.window.window_resolution[0] // 2, self.game.window.window_resolution[1] // 3))
        for char in self.text_to_display:
            if char != " ":
                textbox_surf.blit(pygame.transform.scale_by(font[char], (3, 3)), (0 + x_offset, 0 + y_offset))
                x_offset += (4*3)
            else:
